...
{
    arm_biquad_cascade_df1_init_f32(ctx, ctx->numStages, ctx->pCoeffs, ctx->pState);
    return 0;
}
...
{
    float32_t alpha = 0.16f;
    float32_t a0 = (1.0f - alpha) / 2.0f;
    float32_t a1 = 0.5f;
    float32_t a2 = alpha / 2.0f;
    for (size_t i = 0; i < len; i++)
    {
        window[i] = a0 - a1 * arm_cos_f32(2.0f * PI * i / (len - 1)) + a2 * arm_cos_f32(4.0f * PI * i / (len - 1));
    }
    return 0;
}
...
{
    float32_t binWidth = sampleRate / (float32_t)fftLen;
    for (size_t i = 0; i < fftLen / 2; i++)
    {
        freqBins[i] = i * binWidth;
    }
    return 0;
}
